...
class StableAreaRasterizer:
    ...
    @staticmethod
    def _valid_data_mask(data: np.ndarray, src_nodata) -> np.ndarray:
        """Boolean mask, True where ``data`` is a real measurement.

        Excludes non-finite pixels (NaN / ±inf) and the *input* raster's own
        nodata sentinel.  Without this, an input nodata value that differs from
        ``self.nodata`` and falls inside a stable polygon would be copied through
        verbatim and treated as a real elevation difference by the downstream
        statistics (mean / median-bias / std), silently corrupting them.
        """
    ...
    def rasterize_all(self, output_path: Path | str) -> Path:
        ...
        Inside a stable polygon the original difference value is kept; every
        other pixel (outside the polygons, or an input nodata / non-finite pixel
        inside them) is written as ``self.nodata``.
        """
        ...
        with rasterio.open(self.topo_path) as src:
            ...
            data = src.read(1)
            geoms = [(geom, 1) for geom in self.gdf.geometry
            ...
            keep = (mask == 1) & self._valid_data_mask(data, src.nodata)
            out = np.where(keep, data, self.nodata)
    ...
    def rasterize_each(self, output_dir: Path | str) -> dict[int, Path]:
        ...
        with rasterio.open(self.topo_path) as src:
            ...
            profile.update(nodata=self.nodata)
            data = src.read(1)
            valid = self._valid_data_mask(data, src.nodata)
            for idx, row in self.gdf.iterrows():
                ...
                out = np.where((mask == 1) & valid, data, self.nodata)
...
class StableAreaAnalyzer:
    ...
    def stats_each(self, output_dir: Path | str) -> pd.DataFrame:
        ...
        for area_id, path in paths.items():
            ...
            records.append(df)
        if not records:
            cols = list(descriptive_stats(np.array([], dtype=float)).columns)
            return pd.DataFrame(columns=cols + ['area_id']).set_index('area_id')
