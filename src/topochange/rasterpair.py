...
def _erode_mask(mask: np.ndarray, iterations: int) -> np.ndarray:
    """Shrink a boolean valid-data mask by ``iterations`` pixels (8-connectivity).

    Each iteration drops any pixel 8-adjacent to an invalid pixel or to the
    array edge. Used to strip the thin ring of resampling-contaminated pixels at
    data boundaries: resampling the compare DEM onto the reference grid blends
    real elevations with the nodata sentinel just inside the boundary, producing
    finite-but-wrong values that pass the validity test and appear as an edge
    fringe in the difference. Pure NumPy (no SciPy dependency).
    """
...
class RasterPair:
    ...
    def compute_difference(
        ...
        - Pixel is within ``mask_edge_pixels`` of a boundary in either raster
          (removes the resampling fringe; the difference then shows only pixels
          where both compare and reference are genuinely valid)
        ...
        mask_edge_pixels : int, default 1
            Erode the both-valid mask by this many pixels before differencing, to
            drop the thin ring of resampling-contaminated pixels at data
            boundaries (the edge fringe). 0 disables it (keeps the raw
            intersection); increase to 2+ if a wider fringe remains.
        ...
        # strip the resampling-contaminated edge ring. raster1 is resampled onto
        # raster2's grid, and interpolation just inside the data boundary blends
        # real elevations with the nodata sentinel into finite-but-spurious
        # values that survive the validity test and show up as an edge fringe in
        # the difference. Eroding the both-valid mask keeps only pixels whose
        # full neighbourhood is valid in both rasters.
        if mask_edge_pixels and mask_edge_pixels > 0:
            valid_both = _erode_mask(valid_both, int(mask_edge_pixels))
        ...
        # create CRS history for the difference Raster
    ...
    def compare_raster_metadata(self) -> Dict[str, Any]:
        ...
        Legacy method - see check_all_match() for a fuller comparison.
        """
    ...
    def transform_compare_to_match_reference(
        ...
        if needs_vertical and target_vertical_kind is None:
            raise ValueError(
                ...
                "raster's vertical CRS could not be determined (its header may not "
                "declare one). Declare it before transforming -- e.g. "
                "raster2.add_metadata(vertical_CRS=\"5703\", geoid_model=\"geoid12b\") "
